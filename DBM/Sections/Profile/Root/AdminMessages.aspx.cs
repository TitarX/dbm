...
public partial class Sections_Profile_Root_AdminMessages : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            String selectString = "SELECT MessageID,SubjectText,MessageDate,UserID FROM AdminMessages";
            ...
            while (dataReader.Read())
            {
                ...
                messagesTableBuilder.Append(String.Format("<tr class='rowMessagesTable' onclick='setMessageValues({0},\"{1}\",\"{2}\",\"{3}\",\"{4}\");'>"
                    , messageID, userName, email, subject, messageDate));
                messagesTableBuilder.Append("<td>");
                messagesTableBuilder.Append(userName);
                messagesTableBuilder.Append("</td>");
                messagesTableBuilder.Append("<td>");
                messagesTableBuilder.Append(subject);
                messagesTableBuilder.Append("</td>");
                messagesTableBuilder.Append("<td>");
    ...
    public static String GetMessage(int id)
    {
        ...
        try
        {
            ...
            String selectString = "SELECT MessageText FROM AdminMessages WHERE MessageID=@MessageID";
            SqlCommand selectCommand = new SqlCommand(selectString, connection);
            ...
            String deleteString = "DELETE FROM AdminMessages WHERE MessageID=@MessageID";
